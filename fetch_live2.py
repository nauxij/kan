...
def fetch_sdtv_live():
    """抓取山东广电实时有效m3u8直播流【直接调用jsonAPI，不下载完整HTML】"""
    out = []
    headers = {
        "User-Agent":"Mozilla/5.0 (Android)",
        "Referer":"https://v.iqilu.com/live/yspd/"
    }
    # channel=yspd 对应山东文旅频道
    ...
    try:
        ...
        try:
            ...
            print("打印出错，但直播源变量已经获取：", repr(real_m3u8))

        if real_m3u8:
            out.append(("山东文旅", real_m3u8))
            print(f"✅山东文旅 获取直播流成功")
        else:
            print(f"⚠️山东文旅 api返回没有playUrl")

    except Exception as e:
        print(f"❌山东文旅抓取异常 {repr(e)}")
    return out
